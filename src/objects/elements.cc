...
class ElementsAccessorBase : public InternalElementsAccessor {
 public:
  ...
  static MaybeHandle<FixedArray> PrependElementIndicesImpl(
    ...
    if (nof_elements > FixedArray::kMaxLength - nof_property_keys) {
      THROW_NEW_ERROR(isolate,
                      NewRangeError(MessageTemplate::kInvalidArrayLength));
    }
    size_t initial_list_length = nof_elements + nof_property_keys;
    ...
    combined_keys = Subclass::DirectCollectElementIndicesImpl(
        ...
        combined_keys, static_cast<uint32_t>(nof_elements), &nof_indices);
